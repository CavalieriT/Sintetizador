import numpy as np
import matplotlib.pyplot as plt
from scipy import signal

from audio.filtros import Filtro

fs = 44100
duracion = 0.1
frecuencia = 440

t = np.linspace(0, duracion, int(fs * duracion), endpoint=False)

onda = signal.square(2 * np.pi * frecuencia * t)

filtro = Filtro(
    tipo='bandpass',
    cutoff=1500,
    bandwidth=800,
    sample_rate=fs
)

onda_filtrada = filtro.aplicar(onda)

# FFT
fft_original = np.fft.rfft(onda)
fft_filtrada = np.fft.rfft(onda_filtrada)

freqs = np.fft.rfftfreq(len(onda), 1 / fs)

mag_original = np.abs(fft_original)
mag_filtrada = np.abs(fft_filtrada)

maximo = max(
    np.max(mag_original),
    np.max(mag_filtrada)
)

mag_original /= maximo
mag_filtrada /= maximo

plt.figure(figsize=(10, 6))

plt.plot(freqs, mag_original, label="Original", alpha=0.6)
plt.plot(freqs, mag_filtrada, label="Filtrada (bandpass)", linewidth=2)

# líneas de banda
lowcut = 1500 - 800/2
highcut = 1500 + 800/2

plt.axvline(lowcut, color='red', linestyle='--', label='Banda')
plt.axvline(highcut, color='red', linestyle='--')

plt.xlim(0, 5000)

plt.xlabel("Frecuencia (Hz)")
plt.ylabel("Magnitud normalizada")

plt.title("FFT antes y después del filtro paso banda")

plt.legend()
plt.grid(True)

plt.tight_layout()

plt.show()